  ...
  const create = async () => {
    ...
    try {
      ...
      const res = await apiClient.post("/api/bids", {
        ...
        latitude, longitude, requestNote:note.trim(), deadline,
      });
    ...
    <ScrollView contentContainerStyle={s.content}>
      {!request ? <>
        ...
        {offers.length === 0 ? <View style={s.empty}><Feather name="clock" size={28} color="#94A3B8"/><Text style={s.emptyTitle}>업체의 입찰을 기다리고 있어요</Text></View> :
          offers.map((o:any, i:number) => <View style={s.offer} key={o.id}>
            <Pressable style={s.offerTop} onPress={() => openCompany(o)}>
              <View style={s.rank}><Text style={s.rankText}>{i+1}</Text></View><View style={{flex:1}}>
              <Text style={s.company}>{o.companyName}</Text><Text style={s.meta}>★ {Number(o.avgRating || 0).toFixed(1)} · 후기 {o.reviewCount || 0}개 · {o.distanceKm == null ? "거리 정보 없음" : `${o.distanceKm}km`}</Text></View>
              <View style={s.priceArea}><Text style={s.price}>{won(o.price)}</Text><Feather name="chevron-right" size={18} color="#94A3B8"/></View>
            </Pressable>
...
const s=StyleSheet.create({
  ...
  offer:{backgroundColor:"#fff",borderRadius:18,padding:18,marginBottom:14,borderWidth:1,borderColor:"#E2E8F0"},offerTop:{flexDirection:"row",alignItems:"center",gap:12},rank:{width:34,height:34,borderRadius:17,backgroundColor:"#EEF2FF",alignItems:"center",justifyContent:"center"},rankText:{color:BLUE,fontWeight:"900"},
